library(rjson)
metadata <- fromJSON(file = "metadata.cart.2024-08-12.json")
...
cleanMirFun <- function(fl) {
  ...
  expr <- expr[-(1:4),]
  genenames <- expr[,2]
  expr <- expr$fpkm_unstranded
  names(expr) <- genenames
  return(expr)
}

mRNAMatrix <- lapply(paste('./gdc_download_20240812_030341.579565.tar',metaMatrix$file_id,metaMatrix$file_name,sep = '/'), function(fl) cleanMirFun(fl))

SKCM <- do.call("cbind", mRNAMatrix)
colnames(SKCM) <- metaMatrix$patient
table(metaMatrix$sample_type)

group <- ifelse(as.character(sapply(colnames(SKCM),function(x) substr(x,14,15)))== '01','Tumor','Normal')
write.table(SKCM,'TCGA-SKCM expdata FPKM.txt',sep = '\t')
