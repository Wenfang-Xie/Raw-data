...
gene_set<- read.csv('./pyroptosis.csv',header = T)
gene_set<-gene_set[, 1:2]  
list<- split(as.matrix(gene_set)[,1], gene_set[,2]) 

exprSet <- read.csv("./GSE15605.csv", header=TRUE,row.names=1,check.names = FALSE)
exprSet <- exprSet[,row.names(ann)]

gsva_matrix <- gsva(as.matrix(exprSet), list,method='ssgsea',kcdf='Gaussian',abs.ranking=TRUE)
write.table(gsva_matrix,file = 'ssGSEA_pyroptosis.csv',quote = F,sep = ',')
